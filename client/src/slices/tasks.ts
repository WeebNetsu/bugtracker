 ...
 */
export const updateTask = (taskId: number, update: UpdateTaskModel) => async (dispatch: any) => {
    try {
        // dispatch(updateTaskStarted()); // todo: for some reason this causes update tasks to not work

        if (taskId < 1) {
            throw new Error("Could not find that task, please refresh the page");
        }
 ...
 */
export const deleteSelectedTask = (taskId: number, userId: string) => async (dispatch: any) => {
    try {
